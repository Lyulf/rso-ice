...
CXXFLAGS		= $(STD) $(WARNINGS) -I$(INC_DIR) -DICE_CPP11_MAPPING
...
MASTER_SERVER_SRC	+=main_Server.cpp
MASTER_SERVER_SRC	+=Server.cpp
MASTER_SERVER_SRC	+=$(SLICE_SRC:.ice=.cpp)
MASTER_SERVER_SRC	+=chatServerI.cpp 
MASTER_SERVER_SRC	+=chatRoomFactoryI.cpp

CHILD_SERVER_SRC	+=main_ChatRoomServer.cpp
CHILD_SERVER_SRC	+=ChatRoomServer.cpp
CHILD_SERVER_SRC	+=$(SLICE_SRC:.ice=.cpp)    
CHILD_SERVER_SRC	+=chatRoomI.cpp

CLIENT_SRC		+=main_Client.cpp
CLIENT_SRC		+=Client.cpp
CLIENT_SRC		+=$(SLICE_SRC:.ice=.cpp)
